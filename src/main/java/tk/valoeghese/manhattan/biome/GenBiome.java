...
public final class GenBiome extends BambooJungleBiome {
	...
	public void buildSurface(Random random, Chunk chunk, int x, int z, int worldHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed) {
		SurfaceBuilder<TernarySurfaceConfig> surfaceBuilder = ManhattanProject.netherGen ? SurfaceBuilder.NETHER : SurfaceBuilder.DEFAULT;
		...
		surfaceBuilder.generate(random, chunk, original, x, z, worldHeight, noise, defaultBlock, defaultFluid, seaLevel, seed, config.getSurface(random, x, z, ManhattanProject.netherGen));
	}
